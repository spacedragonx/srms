...
if(!empty($_POST["classid1"])) 
{
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id FROM tbl_dept join  tblsubjects on  tblsubjects.id=tbl_dept.SubjectId WHERE tbl_dept.Dept_id=:cid and tbl_dept.status!=:stts order by tblsubjects.SubjectName");
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT  FROM tblresult WHERE RollId=:id1");
